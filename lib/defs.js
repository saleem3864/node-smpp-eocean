...
var types = {
    ...
    // NEW: Raw buffer type for unprocessed data
    raw_buffer: {
        read: function(buffer, offset) {
            var length = buffer.readUInt8(offset++);
            return buffer.slice(offset, offset + length);
...
filters.message = {
    ...
    decode: function(value, skipUdh) {
        ...
        // NEW: Store raw buffer before decoding
        result.raw_message = Buffer.from(result.message);
...
// NEW: Raw message filter - returns buffer without processing
filters.raw_message = {
    encode: function(value) {
        if (Buffer.isBuffer(value)) {
            return value;
        ...
        if (value && value.message) {
            if (Buffer.isBuffer(value.message)) {
                return value.message;
            }
            return Buffer.from(String(value.message));
        }
        return Buffer.alloc(0);
    },
    decode: function(value) {
        // Always return the raw buffer without any processing
...
var tlvs = {
    ...
    // NEW: Raw message payload TLV
    raw_message_payload: {
        id: 0x0425, // Using next available ID
        type: types.tlv.raw_buffer,
        filter: filters.raw_message
    },
...
var commands = {
    ...
    deliver_sm: {
        ...
        params: {
            ...
            // NEW: Add raw message field
            raw_short_message: {type: types.raw_buffer, filter: filters.raw_message}
        }
    },
    deliver_sm_resp: {
        id: 0x80000005,
...
// NEW: Utility functions for raw buffer access
var rawBufferUtils = {
    /**
     * Extract raw message buffer from PDU
     * @param {Object} pdu - The PDU object
     * @returns {Buffer|null} - Raw message buffer or null if not found
     */
    getRawMessageBuffer: function(pdu) {
        // Try different approaches to get raw buffer
        if (pdu.raw_short_message && Buffer.isBuffer(pdu.raw_short_message)) {
            return pdu.raw_short_message;
        }

        if (pdu.short_message && Buffer.isBuffer(pdu.short_message)) {
            // For regular short_message, extract raw before processing
            if (pdu.esm_class & (consts.ESM_CLASS.UDH_INDICATOR || consts.ESM_CLASS.KANNEL_UDH_INDICATOR)) {
                // Has UDH, return the raw buffer including UDH
                return pdu.short_message;
            } else {
                // No UDH, return the message part directly
                return pdu.short_message;
            }
        }

        // Try TLV message_payload
        if (pdu.message_payload && Buffer.isBuffer(pdu.message_payload)) {
            return pdu.message_payload;
        }

        // Try raw_message_payload TLV
        if (pdu.raw_message_payload && Buffer.isBuffer(pdu.raw_message_payload)) {
            return pdu.raw_message_payload;
        }
    ...
    /**
     * Extract UDH from raw message buffer
     * @param {Buffer} rawBuffer - Raw message buffer
     * @param {number} esmClass - ESM class field
     * @returns {Object} - Object with udh and message parts
     */
    extractUdhFromRaw: function(rawBuffer, esmClass) {
        if (!Buffer.isBuffer(rawBuffer) || rawBuffer.length === 0) {
            return { udh: null, message: rawBuffer };
        }

        var hasUdh = esmClass & (consts.ESM_CLASS.UDH_INDICATOR || consts.ESM_CLASS.KANNEL_UDH_INDICATOR);

        if (hasUdh && rawBuffer.length > 1) {
            var udhLength = rawBuffer[0];
            if (udhLength > 0 && rawBuffer.length > udhLength) {
                return {
                    udh: rawBuffer.slice(0, udhLength + 1),
                    message: rawBuffer.slice(udhLength + 1)
                };
            }
        }
    ...
    /**
     * Get raw message part without UDH
     * @param {Buffer} rawBuffer - Raw message buffer
     * @param {number} esmClass - ESM class field
     * @returns {Buffer} - Raw message without UDH
     */
    getRawMessagePart: function(rawBuffer, esmClass) {
        var extracted = this.extractUdhFromRaw(rawBuffer, esmClass);
        return extracted.message || Buffer.alloc(0);
    },

    /**
     * Get UDH part from raw buffer
     * @param {Buffer} rawBuffer - Raw message buffer
     * @param {number} esmClass - ESM class field
     * @returns {Buffer|null} - UDH buffer or null if no UDH
     */
    getUdhPart: function(rawBuffer, esmClass) {
        var extracted = this.extractUdhFromRaw(rawBuffer, esmClass);
        return extracted.udh;
    },

    /**
     * Check if message has concatenation info
     * @param {Buffer} rawBuffer - Raw message buffer
     * @param {number} esmClass - ESM class field
     * @returns {Object|null} - Concatenation info or null
     */
    getConcatenationInfo: function(rawBuffer, esmClass) {
        var udhBuffer = this.getUdhPart(rawBuffer, esmClass);
        if (!udhBuffer || udhBuffer.length < 6) {
            return null;
        }

        // Skip UDHL (first byte) and look for concatenation headers
        var offset = 1;
        while (offset < udhBuffer.length - 1) {
            var iei = udhBuffer[offset];
            var iedl = udhBuffer[offset + 1];

            // Check for concatenated SMS headers (0x00 or 0x08)
            if ((iei === 0x00 && iedl === 3) || (iei === 0x08 && iedl === 4)) {
                var refNum = (iei === 0x00) ? udhBuffer[offset + 2] : udhBuffer.readUInt16BE(offset + 2);
                var totalParts = udhBuffer[offset + (iei === 0x00 ? 3 : 4)];
                var currentPart = udhBuffer[offset + (iei === 0x00 ? 4 : 5)];
...
exports.rawBufferUtils = rawBufferUtils;