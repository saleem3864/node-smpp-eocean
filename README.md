...
### Raw Buffer Message Support

By default, **node-smpp** does not support handling raw message buffers or GSM 7-bit encoding and decoding.  
Encoding is possible by providing a raw `Buffer` during `submit_sm`, but no such mechanism exists for decoding.  
Hence, this patch is applied to add support for both raw buffer handling and GSM 7-bit decoding.
